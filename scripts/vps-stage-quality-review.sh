...
archive=/home/ubuntu/katalog-quality-stage.tar.gz
...
test "$stage_dir" = /srv/katalog-hoz/staging-quality

echo '[1/7] Cloning production into an isolated staging database'
...
echo '[3/7] Preparing the isolated environment'
...
echo '[4/7] Installing, migrating, and building staging'
...
echo '[5/7] Verifying the isolated database'
runuser -u postgres -- psql -v ON_ERROR_STOP=1 -d "$stage_db" -At <<'SQL'
SELECT 'products=' || count(*) FROM "Product";
SELECT 'review_table=' || count(*) FROM "ProductReview";
...
[Unit]
Description=Katalog Hoz quality review staging
...
echo '[7/7] Staging ready'
echo "stage_service=$(systemctl is-active katalog-hoz-stage)"
echo "stage_http=$(curl -sS -o /dev/null -w '%{http_code}' http://127.0.0.1:3001/admin/product-review)"
